...
const useGetMarketData = ({ vTokenId }: { vTokenId: Token['id'] }) => {
  ...
  return React.useMemo(() => {
    ...
    const underlyingToken = unsafelyGetToken(assetMarket?.underlyingSymbol.toLowerCase() ?? '');
    ...
    const exchangeRateVTokens =
      ...
      new BigNumber(1).div(
        new BigNumber(assetMarket.exchangeRate).div(
          new BigNumber(10).pow(18 + (unsafelyGetToken(vTokenId)?.decimals ?? 0) - (vToken?.decimals ?? 0)),
        ),
      );
