...
import ABIs from 'uniV2ClonesSDK/abis'
...
type DAPPProviderProps = ProviderProps & { children: ReactNode }
export const DappContextProvider: FC<DAPPProviderProps> = ({
  children,
    ...
    useMemo(() => {
      if (config.network && config.exchange) {
        ...
        if (!Object.keys(routerABI || {}).length) {
          throw new Error(`unknown router address, exchange:${config.exchange}`)
        }
        const multicallAddress = getMulticallContractAddress(
          mapThisNetworkNameToMultiChainNetwork(config.network) as NetworkName,
          config.exchange,
        )
