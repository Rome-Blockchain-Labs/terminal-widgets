...
import { Token, Currency, currencyEquals } from 'uniV2ClonesSDK'
import { isAddress } from 'utils/dapp'
...
export const nativeTokenNames = ['AVAX', 'BNB', 'ETH']

export const isNativeToken = (currencyId: string | undefined) =>
  nativeTokenNames.some(t => currencyId?.toUpperCase() === t.toUpperCase())

export function useCurrency(
  currencyId: string | undefined,
  network: NetworkName,
): Currency | null | undefined {
  const isETH = nativeTokenNames.some(
    t => currencyId?.toUpperCase() === t.toUpperCase(),
  )
  const token = useToken(isETH ? undefined : currencyId)
  const nativeToken = getNativeTokenFromNetworkName(network)
  return isETH ? nativeToken : token
}
