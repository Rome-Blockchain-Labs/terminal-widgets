...
export function calculateGasMargin(value: BigNumber): BigNumber {
  ...
  return gasMagin.gte(defaultGasLimitMargin)
    ? value.add(gasMagin)
...
export function getRouterV2Contract(
  chainId: ChainId,
  library: Web3Provider,
  account?: string
): Contract {
  return getContract(
    ROUTER_ADDRESSES_V2[chainId] || '',
    ROUTER_ABI_V2,
    library,
    account
  );
...
export async function getBlocksFromTimestamps(
  ...
  const fetchedData = await splitQuery(
    GET_BLOCKS,
    blockClient[chainId as ChainId],
    [],
    timestamps,
    skipCount
  );
...
export const getTokenSymbol = (token: Token, chainId?: ChainId): string => {
  if (
    ...
    WETH[chainId as ChainId].address.toLowerCase()
  ) {
    switch (chainId) {
      case ChainId.MATIC:
        return 'MATIC';
