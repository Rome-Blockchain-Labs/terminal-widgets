...
export function formatExecutionPrice(
  ...
  return inverted
    ...
    : `${trade.executionPrice.toSignificant(6)} ${nativeOutput?.symbol} / ${
        nativeInput.symbol
      }`;
}

export function computePriceImpactWithoutFee(
  pairs: Pair[],
  priceImpact?: Percent
): Percent | undefined {
  const realizedLPFee: Fraction = computeFee(pairs);
  ...
  // remove lp fees from price impact
  const priceImpactWithoutFeeFraction = priceImpact
    ? priceImpact.subtract(realizedLPFee)
    : undefined;
