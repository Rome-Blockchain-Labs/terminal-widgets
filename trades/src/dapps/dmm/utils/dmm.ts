...
export function priceRangeCalc(
  ...
  //Ex amp = 1.23456
  if (amp && (amp.equalTo(ONE) || amp?.equalTo(ZERO)))
...
export const priceRangeCalcByPair = (
  pair?: Pair
): [Fraction | undefined, Fraction | undefined][] => {
  //Ex amp = 1.23456
...
export const getMyLiquidity = (
  ...
  if (
    !liquidityPosition ||
    parseFloat(liquidityPosition.pool.totalSupply) === 0
  ) {
  ...
  const myLiquidity =
    (parseFloat(liquidityPosition.liquidityTokenBalance) *
      parseFloat(liquidityPosition.pool.reserveUSD)) /
...
export function tokenAmountDmmToSushi(
  amount: TokenAmountDMM
): TokenAmountSUSHI {
...
export function tokenAmountDmmToUni(
  amount: TokenAmountDMM
): TokenAmountUNI | undefined {
...
export function useFarmApr(
  ...
  const yearlyRewardUSD = rewardPerBlocks.reduce(
    (total, rewardPerBlock, index) => {
      if (!rewardPerBlock || !rewardPerBlock.amount) {
        return total;
      }

      if (chainId && tokenPrices[index]) {
        const rewardPerBlockAmount = new TokenAmountDMM(
          rewardPerBlock.token,
          rewardPerBlock.amount.toString()
        );
        const yearlyETHRewardAllocation =
          parseFloat(rewardPerBlockAmount.toSignificant(6)) *
          BLOCKS_PER_YEAR[chainId as ChainId];
...
export function convertToNativeTokenFromETH(
  currency: Currency,
  chainId?: ChainIdDMM
): Currency {
  if (chainId && currency === Currency.ETHER) {
    ...
    if ([97, 56].includes(chainId))
      return new TokenDMM(chainId, WETH[chainId].address, 18, 'BNB', 'BNB');
    if ([43113, 43114].includes(chainId))
      return new TokenDMM(chainId, WETH[chainId].address, 18, 'AVAX', 'AVAX');
    if ([250].includes(chainId))
      return new TokenDMM(chainId, WETH[chainId].address, 18, 'FTM', 'FTM');
  }
...
export function useFarmRewards(
  ...
  const allFarmsRewards = farms.reduce((total, farm) => {
    farm.rewardTokens.forEach((token, index) => {
      if (total[token.address]) {
        total[token.address].amount = total[token.address].amount.add(
          BigNumber.from(farm.lastRewardBlock - farm.startBlock).mul(
            farm.rewardPerBlocks[index]
          )
        );
      } else {
        total[token.address] = {
          amount: BigNumber.from(farm.lastRewardBlock - farm.startBlock).mul(
            farm.rewardPerBlocks[index]
          ),
          token,
        };
  ...
  return onlyCurrentUser
    ? Object.values(userFarmRewards)
    : Object.values(allFarmsRewards);
}
...
export function useFarmRewardsUSD(rewards?: Reward[]): number {
  ...
  const rewardUSD = rewards.reduce((total, reward, index) => {
    ...
    if (chainId && tokenPrices[index]) {
      total +=
        parseFloat(getFullDisplayBalance(reward.amount)) * tokenPrices[index];
    }
...
export function useFarmRewardPerBlocks(farms?: Farm[]): RewardPerBlock[] {
  ...
  const farmRewardPerBlocks = farms.reduce((total, farm) => {
    if (farm.rewardPerBlocks) {
      farm.rewardTokens.forEach((token, index) => {
        if (total[index]) {
          total[index].amount = total[index].amount.add(
...
export function checkIsFarmingPool(
  address: string,
  chainId?: ChainId
): boolean {
  if (!chainId) {
  ...
  return (
    farmingPools.includes(address) ||
    farmingPools.includes(address.toLowerCase())
  );
}
...
export function errorFriendly(text: string): string {
  ...
  if (error.includes('router: expired')) {
    ...
    error.includes('header not found') ||
    error.includes('swap failed') ||
    error.includes('json-rpc error')
  ) {
