  ...
  try {
    ...
    const routesGroup = routes.reduce((a, b) => {
      ...
      if (a[b.slug]) {
        ...
        temp.forEach((sub: any[], ind: number) => {
          ...
          const newSub: any[] = sub.map((pool) => {
            ...
            const percent = new Percent(swapAmount, totalSwapAmount).toFixed(
              0,
              undefined,
          ...
          if (!existed) {
            const percent = new Percent(
              swapPool.swapAmount || ZERO,
              totalSwapAmount
            ).toFixed(0, undefined, Rounding.ROUND_HALF_UP);
      ...
      return Object.assign({}, a, {
        [b.slug]: { index, path: b.path, subRoutes, swapPercentage },
      });
    }, {} as any);

    const routesV2Length = Object.keys(routesGroup).length;
    const routesV2: SwapRouteV2[] = new Array(routesV2Length).map(
      () => ({} as SwapRouteV2)
    );
...
export function getTradeComposition(
  ...
  const calcSwapPercentage = function (
    tokenIn: string,
    amount: string
  ): number | undefined {
    if (!tokenIn || !amount) {
    ...
    const exactTokenIn =
      tokenIn?.toLowerCase() === inputTokenAmount?.token.address?.toLowerCase();
    if (exactTokenIn && trade.inputAmount.greaterThan(ZERO)) {
      const percent = new Percent(
        JSBI.BigInt(amount || 0),
    ...
    if (sorMultiSwap.length === 1) {
      ...
      sorMultiSwap.forEach((hop: any, index: number) => {
        pools.push({
          ...
          swapAmount: JSBI.BigInt(hop.swapAmount),
          swapPercentage:
            index === 0 ? calcSwapPercentage(hop.tokenIn, hop.swapAmount) : 100,
        });
        if (index === 0) {
