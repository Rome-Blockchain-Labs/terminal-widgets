    ...
    rewardLockerAddressesMulticallResult.forEach((address) => {
      if (address?.result) {
        result = result.concat(
          address?.result.filter((item: string) => result.indexOf(item) < 0)
        );
      }
    });
...
export const useRewardTokensByRewardLocker = () => {
  ...
  rewardTokensMulticallResult.forEach((token, index) => {
    fairLaunchToTokensMapping[FAIRLAUNCH_ADDRESSES[chainId as ChainId][index]] =
      token?.result?.[0];
  });

  rewardLockerAddressesMulticallResult.forEach((address, index) => {
    fairLaunchToRewardLockerMapping[
      FAIRLAUNCH_ADDRESSES[chainId as ChainId][index]
    ] = address?.result?.[0];
  });
  ...
  return useMemo(() => {
    ...
    Object.keys(fairLaunchToRewardLockerMapping).forEach(
      (fairLaunchAddress) => {
        const rewardLockerAddress =
          fairLaunchToRewardLockerMapping[fairLaunchAddress];
        ...
        if (result[rewardLockerAddress]) {
          result[rewardLockerAddress] = result[rewardLockerAddress].concat(
            rewardTokens.filter(
              (item: string) => result[rewardLockerAddress].indexOf(item) < 0
            )
          );
        } else {
    ...
    JSON.stringify(fairLaunchToRewardLockerMapping),
    JSON.stringify(fairLaunchToTokensMapping),
  ]);
};
...
export const useSchedules = () => {
  ...
  >((state) => state.dapps.dmm.vesting.schedulesByRewardLocker);
    ...
    const fetchSchedules = async (account: string): Promise<any> => {
      ...
      try {
        ...
        for (let i = 0; i < rewardLockerAddresses.length; i++) {
          ...
          const rewardLockerContract =
            rewardLockerContracts?.[rewardLockerAddress];
          const rewardTokenAddresses =
            rewardTokensByRewardLocker[rewardLockerAddress];
          const rewardTokens = rewardTokensFullInfo.filter((t) =>
            rewardTokenAddresses.includes(t.address)
          );

          const promises = rewardTokens
            .filter((token) => !!token)
            .map(async (token) => {
              const res = await rewardLockerContract?.getVestingSchedules(
                account,
                token.address
              );
