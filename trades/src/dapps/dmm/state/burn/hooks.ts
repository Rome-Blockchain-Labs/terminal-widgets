...
export function useBurnState(): AppState['dapps']['dmm']['burn'] {
  return useSelector<AppState, AppState['dapps']['dmm']['burn']>(
    (state) => state.dapps.dmm.burn
  );
}
...
export function useDerivedBurnInfo(
  ...
  const liquidityValueA =
    ...
    JSBI.greaterThanOrEqual(totalSupply.raw, userLiquidity.raw)
      ? new TokenAmount(
          tokenA,
          pair.getLiquidityValue(tokenA, totalSupply, userLiquidity).raw
        )
      : undefined;
  const liquidityValueB =
    ...
    JSBI.greaterThanOrEqual(totalSupply.raw, userLiquidity.raw)
      ? new TokenAmount(
          tokenB,
          pair.getLiquidityValue(tokenB, totalSupply, userLiquidity).raw
        )
      : undefined;
    ...
    [Field.CURRENCY_A]:
      parsedAmounts && parsedAmounts[Field.CURRENCY_A]
        ? calculateSlippageAmount(
            parsedAmounts[Field.CURRENCY_A] as CurrencyAmount,
            allowedSlippage
          )[0]
        : undefined,
    [Field.CURRENCY_B]:
      parsedAmounts && parsedAmounts[Field.CURRENCY_B]
        ? calculateSlippageAmount(
            parsedAmounts[Field.CURRENCY_B] as CurrencyAmount,
            allowedSlippage
          )[0]
        : undefined,
  };
  ...
  if (
    userLiquidity &&
    parsedAmounts[Field.LIQUIDITY]?.greaterThan(userLiquidity)
  ) {
    error = error ?? 'Insufficient balance';
  }
...
export function useDerivedZapOutInfo(
  ...
  }, [percentToRemove.denominator, percentToRemove.numerator, userLiquidity]);
    ...
    if (independentTokenAmount && liquidityValueA && liquidityValueB) {
      const amount =
        dependentTokenField === Field.CURRENCY_A
          ? JSBI.divide(
              JSBI.multiply(liquidityValueA.raw, percentToRemove.numerator),
              percentToRemove.denominator
            )
          : JSBI.divide(
              JSBI.multiply(liquidityValueB.raw, percentToRemove.numerator),
              percentToRemove.denominator
            );
  ...
  const parsedAmounts: {
    ...
    [Field.LIQUIDITY]:
      userLiquidity && percentToRemove && percentToRemove.greaterThan('0')
        ...
        : undefined,
    [dependentTokenField]:
      tokenIn && dependentTokenAmount
        ? new TokenAmount(tokenIn, dependentTokenAmount.raw)
        : undefined,
    [independentTokenField]:
      tokenOut && independentTokenAmount
        ? new TokenAmount(tokenOut, independentTokenAmount.raw)
        : undefined,
  };
