...
export type TokenAddressMap = Readonly<{
  [chainId in ChainId | number]: Readonly<{
...
export function useAllLists(): {
  ...
  return useSelector<AppState, AppState['dapps']['dmm']['lists']['byUrl']>(
...
export function useAllListsByChainId(): {
  ...
  if (chainId && [ChainId.MATIC, ChainId.MUMBAI].includes(chainId)) {
    ...
    chainId &&
    [ChainId.BSCTESTNET, ChainId.BSCMAINNET].includes(chainId)
  ) {
    lists = Object.keys(allLists)
    ...
    chainId &&
    [ChainId.AVAXTESTNET, ChainId.AVAXMAINNET].includes(chainId)
  ) {
    lists = Object.keys(allLists)
    ...
    lists = Object.keys(allLists)
      .filter(
        (key) =>
          !MATIC_TOKEN_LISTS.includes(key) &&
          !BSC_TOKEN_LISTS.includes(key) &&
          !AVAX_TOKEN_LISTS.includes(key)
      )
      .reduce((obj, key) => {
        obj[key] = allLists[key];
        return obj;
...
export function useActiveListUrls(): string[] | undefined {
  const activeListUrls = useSelector<
    AppState,
    AppState['dapps']['dmm']['lists']['activeListUrls']
  >((state) => state.dapps.dmm.lists.activeListUrls);
...
// get all the tokens from networkLogos lists, combine with local default tokens
