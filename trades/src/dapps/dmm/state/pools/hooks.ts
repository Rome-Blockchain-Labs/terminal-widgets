...
import { useExchangeClient } from '../application/hooks';
...
): UserLiquidityPositionResult {
  const { data, error, loading } = useQuery(USER_POSITIONS, {
...
function parseData(
  ...
  // get volume changes
  const oneDayVolumeUSD = get24hValue(
    data?.volumeUSD,
    oneDayData?.volumeUSD ? oneDayData.volumeUSD : 0
  );

  const oneDayFeeUSD = get24hValue(
    data?.feeUSD,
...
export function useBulkPoolData(
  ...
  loading: AppState['dapps']['dmm']['pools']['loading'];
  error: AppState['dapps']['dmm']['pools']['error'];
  ...
  const poolsData = useSelector(
    (state: AppState) => state.dapps.dmm.pools.pools
  );
  const loading = useSelector(
    (state: AppState) => state.dapps.dmm.pools.loading
  );
  const error = useSelector((state: AppState) => state.dapps.dmm.pools.error);
    ...
    async function checkForPools() {
      try {
        if (poolList.length > 0 && !error && poolsData.length === 0) {
          ...
          const pools = await getBulkPoolData(
            poolList as string[],
            apolloClient,
            ethPrice,
            chainId
  ...
  return { data: poolsData, error, loading };
}

export function useResetPools(
...
export function useSelectedPool() {
  return useSelector((state: AppState) => state.dapps.dmm.pools.selectedPool);
}
