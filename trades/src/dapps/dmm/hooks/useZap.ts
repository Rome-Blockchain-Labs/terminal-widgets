...
const useZap = () => {
  ...
  const calculateZapInAmounts = useCallback(
    async (
      ...
      try {
        const result = await zapContract?.calculateZapInAmounts(
          tokenIn,
          tokenOut,
          pool,
          userIn
  ...
  const calculateZapOutAmount = useCallback(
    async (
      ...
      try {
        const result = await zapContract?.calculateZapOutAmount(
          tokenIn,
          tokenOut,
          pool,
          lpQty
    ...
    async function handleCalculateZapInAmounts() {
      ...
      try {
        if (tokenIn && tokenOut && pool && userIn?.gt(0)) {
          const amounts = await calculateZapInAmounts(
            tokenIn,
            tokenOut,
            pool,
            userIn
    ...
    async function handleCalculateZapOutAmount() {
      ...
      try {
        if (tokenIn && tokenOut && pool && lpQty?.gt(0)) {
          const amount = await calculateZapOutAmount(
            tokenIn,
            tokenOut,
            pool,
            lpQty
