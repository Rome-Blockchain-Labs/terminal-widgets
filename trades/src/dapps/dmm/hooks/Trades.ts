...
function useAllCommonPairs(
  ...
  const bases: Token[] = chainId
    ? BASES_TO_CHECK_TRADES_AGAINST[
        chainId as keyof typeof BASES_TO_CHECK_TRADES_AGAINST
      ]
    : [];
      ...
      tokenA && tokenB
        ? [
            ...
            .filter(([tokenA, tokenB]) => {
              ...
              const customBases =
                CUSTOM_BASES[chainId as keyof typeof CUSTOM_BASES];
...
export function useTradeExactOut(
  ...
  }, [allowedPairs, currencyAmountOut, currencyIn]);
  return trade;
  // return useMemo(() => {
...
export function useTradeExactInV2(
  ...
  const debouncedCurrencyAmountIn = useDebounce(
    currencyAmountIn?.toSignificant(10),
    300
  );
  const debouncedCurrencyIn = useDebounce(currencyAmountIn?.currency, 300);

  const routerApi = useMemo((): string => {
    return (chainId && routerUri[chainId as keyof typeof routerUri]) || '';
  }, [chainId]);

  const gasPrice = useSelector(
    (state: AppState) => state.dapps.dmm.application.gasPrice
  );
  const onUpdateCallback = useCallback(async () => {
    if (currencyAmountIn && currencyOut) {
      ...
      setComparer(null);
      setTrade(state);
      const comparedResult = await Aggregator.compareDex(
        routerApi,
        currencyAmountIn,
        currencyOut
      );
