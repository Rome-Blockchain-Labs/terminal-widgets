...
export const getTokenDetailQuery = (blockchain: string) => gql`
  query TokenDetails(
    $pair_address: String!
    $exchange: String!
  ) @cached {
    pair: ${blockchain}_pair_summary(
      where: {
    ...
    all_txs: ${blockchain}_get_pair_tx_count(args: {_exchange: $exchange, _pair_address:$pair_address}) {
      tx_count
    }
  }
`;
